...
Compute:  
\( C' = \{ h_{c_1}, h_{c_2}, \dots, h_{c_v} \} \), where \( h_{c_i} = H(c_i) \)

For each \( i \), choose  \( R_i \xleftarrow{R} \mathbb{Z}_n^* \)

Compute:  \( y_i = h_{c_i} \cdot (R_i)^e \mod n \)

Client sends:  \( \{ y_1, y_2, \dots, y_v \} \)

After receiving \( y'_i \):
- \( K_{c_i} = y'_i \cdot R_i^{-1} \mod n \)
- \( t'_i = H'(K_{c_i}) \)
